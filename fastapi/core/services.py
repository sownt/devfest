...
class BaseServices:
    ...
    def get_session(self, commons: CommonsDependencies) -> Session:
        return commons.session
    ...
    async def get_by_id(self, _id, fields_limit: list | str = None, ignore_error: bool = False, include_deleted: bool = False, commons: CommonsDependencies = None) -> dict:
        ...
        session = self.get_session(commons=commons)
        query = {}
        ...
        item = await self.crud.get_by_id(_id=_id, session=session, fields_limit=fields_limit, query=query)
    ...
    async def get_all(
        ...
        item = await self.crud.get_all(session=session, query=query, search=search, search_in=search_in, page=page, limit=limit, fields_limit=fields_limit, sort_by=sort_by, order_by=order_by)
        return item

    async def get_by_field(
        self, data: str, field_name: str, fields_limit: list | str = None, ignore_error: bool = False, include_deleted: bool = False, commons: CommonsDependencies = None) -> dict:
        """
        ...
        if ownership_query:
            query.update(ownership_query)
        item = await self.crud.get_by_field(data=data, field_name=field_name, session=session, fields_limit=fields_limit, query=query)
        if not item and not ignore_error:
    ...
    async def save(self, data: dict, commons: CommonsDependencies) -> dict:
        """
        ...
        self.ensure_crud_provided()
        session = self.get_session(commons=commons)
        item = await self.crud.save(data=data, session=session)
        result = await self.get_by_id(_id=item, commons=commons)
        return result
    ...
    async def update_by_id(
        ...
        await self.crud.update_by_id(_id=_id, data=data, session=session)
        result = await self.get_by_id(_id=_id, ignore_error=ignore_error, include_deleted=True, commons=commons)
        return result
    ...
    async def hard_delete_by_id(self, _id: str,  ignore_error: bool = False, include_deleted: bool = False, commons: CommonsDependencies = None) -> bool:
        ...
        result = await self.crud.delete_by_id(_id=_id, session=session)
        if not result:
            raise CoreErrorCode.NotFound(service_name=self.service_name, item=_id)
        return result
