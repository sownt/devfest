...
class RoutersCBV:
    ...
    @router.post("/users/admin", status_code=201, responses={201: {"model": schemas.LoginResponse, "description": "Register user success"}})
    async def create_admin(self):
        result = await user_controllers.create_admin(commons=self.commons)
    ...
    async def get_detail(self, _id: str, fields: str = None):
    ...
    async def register(self, data: schemas.RegisterRequest):
        result = await user_controllers.register(data=data, commons=self.commons)
        return schemas.LoginResponse(**result)
    ...
    async def login(self, data: schemas.LoginRequest):
        result = await user_controllers.login(data=data, commons=self.commons)
        return schemas.LoginResponse(**result)

    @router.put("/users/{_id}", status_code=200, responses={200: {"model": schemas.Response, "description": "Update user success"}})
    async def edit(self, _id: str, data: schemas.EditRequest):
